...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    if (gaugesEl) {
      if (!ongoingForGauges.length) {
        ...
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          ...
          return `<div style="text-align:center;min-width:130px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 120 70" width="130" height="76" style="overflow:visible;display:block;margin:0 auto">
              <defs>
                <filter id="glow-${i}"><feGaussianBlur stdDeviation="3.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
              </defs>
              <!-- Single glowing arc — no background track -->
              <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="9" stroke-linecap="round" filter="url(#glow-${i})"/>
              <!-- Needle stem -->
              <path d="${needleStem}" stroke="${arrowCol}" stroke-width="1.8" stroke-linecap="round" opacity="0.85"/>
              <!-- Needle arrowhead — limit color (white) -->
              <path d="${arrowHead}" fill="${arrowCol}" style="filter:drop-shadow(0 0 3px ${arrowCol})"/>
              <!-- Pivot -->
              <circle cx="${cx}" cy="${cy}" r="3.5" fill="${arrowCol}" stroke="rgba(0,0,0,.6)" stroke-width="1.2"/>
              <!-- Value -->
              <text x="${cx}" y="${cy-10}" text-anchor="middle" font-size="12" font-weight="800" fill="${col}" font-family="monospace">+${Math.round(ageDays)}d</text>
            </svg>
            <div style="font-size:.62rem;font-weight:700;color:${col};margin-top:.05rem;letter-spacing:.5px">△ +${Math.round(ageDays)}d</div>
            <div style="font-size:.56rem;color:rgba(255,255,255,.55);margin-top:.12rem;font-weight:600">${r['Tender No']||'—'}</div>
            <div style="font-size:.52rem;color:rgba(255,255,255,.35);margin-top:.06rem">${poleType||'—'} · ${r['Site Name']||'—'}</div>
          </div>`;
        }).join('');
    ...
    function buildDelayMeter(delayVal, rowIdx) {
      ...
      const needleLen=rad-7, arrowSize=5;
      ...
      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}" style="text-align:center;min-width:130px;">
        <svg viewBox="0 0 120 70" width="130" height="76" style="overflow:visible;display:block;margin:0 auto">
          <defs><filter id="${filtId}"><feGaussianBlur stdDeviation="3.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
          <path d="${singleArc}" fill="none" stroke="${col}" stroke-width="9" stroke-linecap="round" filter="url(#${filtId})"/>
          <path d="${needleStem}" stroke="${arrowCol}" stroke-width="1.8" stroke-linecap="round" opacity="0.85"/>
          <path d="${arrowHead}" fill="${arrowCol}" style="filter:drop-shadow(0 0 3px ${arrowCol})"/>
          <circle cx="${cx}" cy="${cy}" r="3.5" fill="${arrowCol}" stroke="rgba(0,0,0,.6)" stroke-width="1.2"/>
          <text x="${cx}" y="${cy-10}" text-anchor="middle" font-size="12" font-weight="800" fill="${col}" font-family="monospace">${valTxt}</text>
        </svg>
        <div style="font-size:.62rem;font-weight:700;color:${lblCol};margin-top:.05rem;letter-spacing:.5px">${lblTxt}</div>
      </div>`;
    }
